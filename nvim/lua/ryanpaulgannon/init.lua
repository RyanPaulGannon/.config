-- core
require("ryanpaulgannon.core.keymaps")
require("ryanpaulgannon.core.options")

-- plugins
require("ryanpaulgannon.plugins.telescope")
require("ryanpaulgannon.plugins.rose-pine")
require("ryanpaulgannon.plugins.treesitter")
require("ryanpaulgannon.plugins.lsp")
require("ryanpaulgannon.plugins.fugitive")
require("ryanpaulgannon.plugins.autocomplete")
require("ryanpaulgannon.plugins.format")
require("ryanpaulgannon.plugins.lint")
require("ryanpaulgannon.plugins.harpoon")
require("ryanpaulgannon.plugins.tabnine")
require("ryanpaulgannon.plugins.comment")
